module github.com/onosproject/onos-kpimon

go 1.14

require (
	github.com/envoyproxy/protoc-gen-validate v0.6.1 // indirect
	github.com/onosproject/onos-api/go v0.7.45
	github.com/onosproject/onos-e2-sm/servicemodels/e2sm_kpm v0.7.26 // indirect
	github.com/onosproject/onos-e2-sm/servicemodels/e2sm_kpm_v2 v0.7.26
	github.com/onosproject/onos-e2t v0.7.13
	github.com/onosproject/onos-lib-go v0.7.7
	github.com/onosproject/onos-ric-sdk-go v0.7.11
	github.com/stretchr/testify v1.7.0 // indirect
	google.golang.org/grpc v1.33.2
	google.golang.org/protobuf v1.26.0
)

replace github.com/onosproject/onos-ric-sdk-go => /Users/adibrastegarnia/go/src/github.com/onosproject/onos-ric-sdk-go
